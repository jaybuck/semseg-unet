...
from generator_exr import ExrDataGenerator
...
if __name__ == '__main__':
    ...
    train_filepaths = listdir_files(train_dir)
    train_filenames = list(train_filepaths.keys())
    n_train = len(train_filenames)
    print('type train_filepaths {}    train_filenames: {}'.format(type(train_filepaths), type(train_filenames)))
    print('Training:  dir: {}  Number of images: {}'.format(train_dir, n_train))

    val_filepaths = listdir_files(test_dir)
    val_filenames = list(val_filepaths.keys())
    ...
    train_generator = ExrDataGenerator(train_filenames, train_filepaths,
                                       to_fit=True, batch_size=batch_size, pos_weight=pos_weight, verbosity=2)

    val_generator = ExrDataGenerator(val_filenames, val_filepaths,
