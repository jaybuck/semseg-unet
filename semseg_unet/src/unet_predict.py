...
if __name__ == '__main__':
    ...
    img_generator = ExrDataGenerator(img_filenames, img_filepaths, shuffle=False,
    ...
    for batch_index in range(steps_per_epoch):
        ...
        for i, pred in enumerate(preds):
            ...
            pred_buf = nparray_to_image_buf(model_score, img_buf_type=oiio.FLOAT, channel_names=(prob_label,))
            pred_spec = pred_buf.spec()
            # print('pred_spec', get_imagebuf_info(pred_buf))
            ...
            #pred_prob_buf = oiio.ImageBufAlgo.channel_append(exr_buf, pred_buf)
            # pred_prob_buf = concatenate_images(exr_buf, pred_buf)
